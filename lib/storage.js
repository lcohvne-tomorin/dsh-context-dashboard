...
/**
 * 单价覆盖白名单：每个值 ≥0 且为有限数；整条非法即拒绝（§7.4）。
 * 键为 `<channel>/<model>`：两者都走受限字符集 + 长度上限（仅作对象键使用，
 * 不参与路径拼接）；保留 `. _ - : + @` 等真实模型 id 常见字符，避免「录了价却被丢弃」。
 */
export function sanitizeOverrides(raw) {
  const out = {}
  const src = (raw && typeof raw === 'object') ? raw : {}
  for (const [key, rates] of Object.entries(src)) {
    if (!/^[A-Za-z0-9._:+-]{1,120}\/[A-Za-z0-9._:@+-]{1,160}$/.test(key)) continue
