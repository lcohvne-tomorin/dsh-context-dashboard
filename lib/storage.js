...
const DEFAULTS = {
  ...
  balance: {
    ...
    keys: {
      'qwen-token-plan-cn': { mode: 'env', envName: 'QWEN_TOKEN_PLAN_CN_API_KEY' },
      'opencode-go-full': { mode: 'env', envName: 'OPENCODE_GO_FULL_API_KEY' },
      'deepseek-official': { mode: 'env', envName: 'DEEPSEEK_API_KEY' },
    },
    refreshMs: 30000,
  },
...
export function sanitizeConfig(raw) {
  ...
  // keys：只接受 { channel: { mode:'env', envName } }，envName 白名单字符集
  const keySrc = (bk.keys && typeof bk.keys === 'object') ? bk.keys : {}
  for (const ch of ['qwen-token-plan-cn', 'opencode-go-full', 'deepseek-official']) {
    const k = keySrc[ch]
    if (k && k.mode === 'env' && /^[A-Za-z_][A-Za-z0-9_]*$/.test(String(k.envName || ''))) {
      cfg.balance.keys[ch] = { mode: 'env', envName: String(k.envName) }
    } else if (cfg.balance.keys[ch] === undefined) {
      // 保留默认 env 名
    }
  }
  cfg.balance.keys = { ...DEFAULTS.balance.keys, ...cfg.balance.keys }
