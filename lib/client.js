...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		var zh = {
			...
			cfgBalance: "余额查询",
			cfgBalanceDesc: "开启后向渠道官方账户接口查询余额/配额。",
			cfgBalanceNoChannels: "模型选择器暂无可用渠道——配置好 LLM 渠道后回到本页即可看到。",
			cfgKeySource: "密钥来源（环境变量）",
			cfgKeySourceDesc: "key 只从环境变量读取，绝不落盘、不进日志、不下发界面。",
			cfgOverrides: "单价覆盖表",
			cfgOverridesDesc: "覆盖官方计价目录（每百万 token）。留空该格则沿用目录。未收录的估算值也在此校准。模型清单与模型选择器同源（新加的模型可直接选）。",
			cfgNoModels: "该渠道暂无可用模型",
			cfgNoPrice: "未定价",
			cfgPickModel: "选择模型…",
		...
		var en = {
			...
			cfgBalanceDesc: "Query the channel's official account API for balance/quota.",
			cfgBalanceNoChannels: "No channels available in the model selector — configure an LLM channel and return here.",
			...
			cfgOverridesDesc: "Override the official pricing catalog (per 1M tokens). Leave a rate blank to keep the catalog. Unverified estimates are calibrated here. Models mirror the model selector, so newly added ones are selectable.",
			cfgNoModels: "No models available for this channel",
			cfgNoPrice: "no price yet",
			cfgPickModel: "Pick a model…",
		...
		function SettingsSection(props) {
			...
			var modelsS = React.useState(null);
			var models = modelsS[0]; var setModels = modelsS[1];
			...
			/* ---- 余额 + key env（渠道 = 模型选择器可用渠道，host 半动态枚举，不预设） ---- */
			var balFields = Object.keys(channels || {}).map(function (ch) {
				...
				return h("div", { className: "cd-field", key: ch },
					h("span", { className: "cd-flabel" }, (meta.display || ch) + " (" + (meta.kind || "unknown") + ")"),
					h("input", {
						className: "cd-input", type: "text", value: k.envName || "",
						placeholder: meta.suggestEnv || "ENV_VAR_NAME",
			...
			var balList = balFields.length > 0
				? h("div", { className: "cd-body", style: { gap: "8px" } }, balFields)
				: h("p", { className: "cd-fdesc" }, t0("cfgBalanceNoChannels"));
			...
			/* 模型清单与模型选择器同源（host 半按渠道路由 `llm.listModels` 动态枚举），
			   与内置目录取并集：新加的模型可选，目录里的旧款也保留（便于预置价格）；
			   llm 不可用时退化为纯目录行（旧行为）。 */
			var liveForChannel = (models && models[add.channel]) || [];
			var seenModel = {};
			var mergedModels = [];
			liveForChannel.forEach(function (m) {
				if (m && m.id && !seenModel[m.id]) { seenModel[m.id] = true; mergedModels.push({ id: m.id, name: m.name || m.id, live: true }); }
			});
			(cat || []).filter(function (r) { return !add.channel || r.channel === add.channel; }).forEach(function (r) {
				if (!seenModel[r.model]) { seenModel[r.model] = true; mergedModels.push({ id: r.model, name: r.name || r.model, live: false }); }
			});
			var modelOptions = mergedModels.map(function (m) {
				return h("option", { key: (m.live ? "live:" : "cat:") + m.id, value: m.id }, (m.name && m.name !== m.id) ? (m.name + " · " + m.id) : m.id);
			});
			if (modelOptions.length === 0) modelOptions = [h("option", { key: "none", value: "" }, t0("cfgNoModels"))];
			// 占位项：未选模型时显示与状态一致（否则 React 会显示第一项，而按钮因 model 为空而禁用）
			else modelOptions = [h("option", { key: "ph", value: "" }, t0("cfgPickModel"))].concat(modelOptions);
			var addRow = h("div", { className: "cd-ovrow" },
				h("div", { className: "cd-field" },
					h("select", { className: "cd-select", value: add.channel, onChange: function (e) { setAdd(Object.assign({}, add, { channel: e.target.value, model: "" })); } },
						Object.keys(channels || {}).map(function (ch) { return h("option", { key: ch, value: ch }, (channels[ch] && channels[ch].display) || ch); }))),
				h("div", { className: "cd-field" },
			...
			/* ---- 目录只读表（含 host 半从模型选择器带出的「新模型」行，按当前生效价展示） ---- */
			var catRows = (cat || []).map(function (r) {
				var label = (r.name && r.name !== r.model) ? (r.model + " · " + r.name) : r.model;
				var dash = t0("errUnknown");
				var status = !r.rates ? t0("cfgNoPrice") : (r.verified ? t0("verified") : t0("unverified"));
				if (r.rates && r.window === "offpeak") status += " · " + t0("offpeak");
				return h("tr", { key: r.channel + "/" + r.model + (r.live ? ":live" : "") },
					h("td", null, r.channel),
					h("td", null, label),
					h("td", null, r.rates ? String(r.rates.input) : dash),
					h("td", null, r.rates ? String(r.rates.cacheRead) : dash),
					h("td", null, r.rates ? String(r.rates.output) : dash),
					h("td", null, r.currency || dash),
					h("td", null, status));
			});
