...
// 各渠道允许的 env key 名（与 storage.js 的默认值一致；实际取值按设置页配置）
const ENV_KEYS = ['QWEN_TOKEN_PLAN_CN_API_KEY', 'OPENCODE_GO_FULL_API_KEY', 'DEEPSEEK_API_KEY']
...
export default {
  ...
  apply(ctx) {
    ...
    log(`[${NAME}] active (routes=${BASE}, channels=${Object.keys(pricing.CHANNELS).join(',')})`)
    const limiter = {
    ...
    async function channelKey(channel, c) {
      ...
      if (!keyCfg || keyCfg.mode !== 'env' || typeof keyCfg.envName !== 'string' || keyCfg.envName.length === 0) return undefined
      const ref = keyCfg.envName
      try {
    ...
    async function ensureBalances(c) {
      for (const channel of Object.keys(pricing.CHANNELS)) {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (req.method === 'GET' && pathname === `${BASE}/config`) {
            ...
            httpkit.sendJson(res, 200, { ok: true, csrf: csrfToken, config: cfg(), fold: fold === undefined ? null : { collapsed: fold }, channels: pricing.CHANNELS, catalog: pricing.CATALOG, meta: pricing.CATALOG_META }); return
          }
          ...
          if (req.method === 'POST' && pathname === `${BASE}/refresh-balance`) {
            ...
            const balances = {}
            for (const channel of Object.keys(pricing.CHANNELS)) balances[channel] = balanceCache.get(channel).payload
