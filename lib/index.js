...
export default {
  ...
  apply(ctx) {
    ...
    /**
     * 各渠道当前可用模型（与模型选择器同源）：host `llm.listModels(channelId)`。
     * llm 服务缺失时返回 null（调用方回退内置目录）；单渠道失败只丢该渠道。
     * @returns { Promise<{ [channelId]: Array<{id:string,name:string}> } | null> }
     */
    async function liveModels() {
      let llm = null
      try { llm = typeof ctx.get === 'function' ? ctx.get('llm') : undefined } catch { llm = null }
      ...
      for (const id of liveChannelIds()) {
        ...
        } catch { /* 单渠道失败：该渠道回退目录展示 */ }
      }
      return out
    }

    /**
     * 计价目录行：内置 CATALOG + 「活体渠道里有、目录里没有」的模型（用当前生效价，
     * 未校准即 fallback 估算），使设置页能对**新加的模型**录入单价覆盖。
     * @param {object} c - 已清洗的配置（覆盖表参与取价）
     * @param {object|null} models - liveModels() 结果
     */
    function catalogRows(c, models) {
      const rows = pricing.CATALOG.map((r) => ({ ...r, live: false }))
      if (!models) return rows
      const known = new Set(pricing.CATALOG.map((r) => `${r.channel}/${r.model}`))
      for (const [channel, list] of Object.entries(models)) {
        for (const m of list) {
          const key = `${channel}/${m.id}`
          if (known.has(key)) continue
          const resolved = pricing.resolvePrice(channel, m.id, c.priceOverrides)
          const meta = pricing.channelMeta(channel)
          // 未收录又无兜底价的渠道（如自建网关）也要出行：面板给不出价，
          // 但设置页必须能对它录入覆盖价，否则永远改不了费用。
          rows.push({
            channel,
            model: m.id,
            name: m.name,
            ...
            currency: resolved.price ? resolved.price.currency : (meta.currency || null),
            rates: resolved.price
              ? {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (req.method === 'GET' && pathname === `${BASE}/config`) {
            ...
            httpkit.sendJson(res, 200, { ok: true, csrf: csrfToken, config: c, fold: fold === undefined ? null : { collapsed: fold }, channels: liveChannels(), models, catalog: catalogRows(c, models), meta: pricing.CATALOG_META }); return
          }
