...
- **新加的模型在「单价覆盖表」里选不到（列表仍是旧的）**：覆盖表的模型下拉此前取自内置静态
  目录 `pricing.CATALOG`，与模型选择器无关。现改为 host 半按渠道 `ctx.llm.listModels()`
  动态枚举（`/config` 新增 `models` 字段），并与目录**取并集**：新加的模型可直接选到，
  目录里的旧款同时保留以便预置价格；`llm` 不可用时退化为纯目录（旧行为，测试同此路径）。
- **只读目录表看不到新模型**：把「活体渠道里有、内置目录里没有」的模型补进目录表，按当前
  生效价展示（含覆盖值），无兜底价的渠道（如自建网关）标「未定价」但仍出行——否则无法
  为它录入单价。
- **覆盖表键白名单过窄导致「录了价不生效」**：`<channel>/<model>` 键此前仅允许
  `A-Za-z0-9._-`，真实模型 id 里的 `:`/`+`/`@` 会被静默丢弃。现放宽为受限字符集 + 长度
  上限（§7.4 仍为白名单，键仅作对象键使用、不参与路径拼接）。
- **覆盖表「选了模型却不能添加」的观感问题**：未选模型时 React 会显示第一项，而按钮因
  `model` 为空而禁用。现加「选择模型…」占位项，显示与状态一致。
- 测试：集成 +1（动态模型枚举 / 无兜底渠道出行 / 覆盖价回显）、单测 +1（覆盖键白名单）。
